...
class File(Base):
    ...
    # ── Fields added by migration a0a725601d8a ────────────────────────────────
    # These columns exist in the DB. They were previously missing from the ORM
    # class, causing f.importance_score = ... to set a plain Python attribute
    # with no DB mapping, and self.db.flush() to raise InvalidRequestError.
    importance_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    summary_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    responsibilities: Mapped[str | None] = mapped_column(Text, nullable=True)
    imports_list: Mapped[str | None] = mapped_column(Text, nullable=True)
    exports_list: Mapped[str | None] = mapped_column(Text, nullable=True)
    framework_hints: Mapped[str | None] = mapped_column(Text, nullable=True)
    # ─────────────────────────────────────────────────────────────────────────
